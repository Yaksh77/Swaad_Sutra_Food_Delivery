  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await axios.put(
        `${SERVER_API}/item/edit-item/${itemId}`,
        formData,
        {
    ...
    const handleGetItemById = async () => {
      try {
        const result = await axios.get(
          `${SERVER_API}/item/get-item/${itemId}`,
          {
            withCredentials: true,
          }
        );
    ...
    <div className="flex justify-center flex-col items-center p-6 bg-gradient-to-br from-green-50 relative to-white min-h-screen">
      ...
      <div className="max-w-lg w-full bg-white shadow-xl rounded-2xl p-8 border border-green-100">
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          ...
          <button
            disabled={loading}
            className="w-full text-lg bg-gray-500 text-white px-6 py-2 rounded-lg font-semibold shadow-md hover:bg-green-700 hover-shadow-lg transition-all cursor-pointer"
          >
