...
export const searchItems = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Error occurred while searching the items" });
  }
};
...
export const rating = async (req, res) => {
  try {
    ...
    if (rating < 1 || rating > 5) {
      return res.status(400).json({ message: "rating must between 1 To 5" });
    }
    ...
    return res
      .status(500)
      .json({ message: "Error occurred while processing the rating" });
  }
};
